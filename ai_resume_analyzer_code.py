# -*- coding: utf-8 -*-
"""AI_Resume_Analyzer_code

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gD8LGTqF8u_7d9mhYgaul-r6Dk_nR1jg

# 🤖 AI Resume Analyzer — Evidence-Based Category & Job Match
"""

# CELL 1: INSTALL REQUIRED LIBRARIES
!pip install -q pymupdf scikit-learn
print("Libraries installed")

"""## Resume Analyzer Engine"""

# CELL 2: RESUME ANALYZER ENGINE
"""
Resume Analyzer Engine
======================
...
resume_file_name = list(uploaded_resume.keys())[0]

resume_text = extract_text_from_pdf(resume_file_name)
print("Extracted", len(resume_text), "characters")
print(resume_text[:1000])

# CELL 4: PREDICT CATEGORY (evidence-based, not a guess)
...
if result["status"] == "low_confidence":
    ...
    for c in result["top_candidates"]:
        print(f"  - {c['category']}: {c['raw_score']} evidence points")
else:
    print(f"Predicted Category: {result['predicted_category']}  ({result['confidence_pct']}% confidence)")
    print()
    print("Evidence used:")
    print("  Title phrases matched:", result["evidence"]["matched_title_phrases"])
    print("  Skills matched:", result["evidence"]["matched_skills"])
    print()
    print("Other candidates considered:")
    for c in result["top_candidates"][1:]:
        print(f"  - {c['category']}: {c['confidence_pct']}%  (matched: {c['matched_skills']})")

# CELL 5: PASTE THE JOB DESCRIPTION
job_description = input("Paste the Job Description here:\n")
print("Job description received!")

# CELL 6: RESUME <-> JOB DESCRIPTION MATCH
match = match_resume_to_job(resume_text, job_description)

print("=" * 60)
print("        RESUME-JOB MATCH REPORT")
print("=" * 60)
print(f"\nOverall Match Score: {match['overall_match_pct']}%")
print(f"Verdict: {match['verdict']}")
print(f"  (Skill overlap: {match['skill_overlap_pct']}%, Content similarity: {match['text_similarity_pct']}%)")

print(f"\nExperience - Resume: {match['resume_years_experience']} yrs | JD requires: {match['jd_years_experience_required']} yrs")
...
print("\nBonus Skills (you have, JD didn't ask for):")
...
!ls -la app.py resume_analyzer_engine.py
